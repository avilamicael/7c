...
const STATUS_CONFIG = {
  ...
  PAGA:               { label: "Paga",                 className: "bg-emerald-100 text-emerald-700 border-emerald-200" },
  VENCIDA:            { label: "Vencida",              className: "bg-red-100 text-red-700 border-red-200" },
  PARCIALMENTE_PAGA:  { label: "Parcialmente Paga",    className: "bg-blue-100 text-blue-700 border-blue-200" },
...
export function ContasPagarTable({ filters }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.busca) {
      ...
      result = result.filter((c) => c.fornecedor_nome?.toLowerCase().includes(q) || c.numero_documento?.toLowerCase().includes(q));
    }
        ...
        <TableBody>
          {filtered.length === 0 ? (
            ...
            <TableRow key={conta.public_id} className="hover:bg-muted/30 transition-colors">
              ...
              <TableCell>{`${(conta.parcelas ?? []).filter((p) => p.status === "PAGA").length}/${conta.total_parcelas}`}</TableCell>
