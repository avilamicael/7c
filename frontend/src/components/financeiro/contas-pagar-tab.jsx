...
import { DetalhesContaPagarModal } from "@/components/financeiro/detalhes-conta-pagar-modal";
import { EditContaPagarModal } from "@/components/financeiro/edit-conta-pagar-modal";
import { Search, Plus, Eye, Pencil, Trash2, DollarSign, AlertTriangle, CheckCircle, FileText } from "lucide-react";
import { cn } from "@/lib/utils";

const STATUS_OPTIONS = ["Todos", "PENDENTE", "PAGA", "VENCIDA", "PARCIALMENTE_PAGA", "CANCELADA", "BAIXA_MANUAL"];
const STATUS_LABEL = {
...
const STATUS_COLOR = {
  ...
  PAGA: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-900",
  VENCIDA: "bg-red-50 text-red-600 border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-900",
  PARCIALMENTE_PAGA: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/40 dark:text-blue-400 dark:border-blue-900",
...
function calcKPIs(contas) {
  ...
  let totalPendente = 0, totalVencido = 0, totalPagoNoMes = 0;
  for (const c of contas) {
    for (const p of c.parcelas ?? []) {
      ...
      if (p.status === "PENDENTE" || p.status === "PARCIALMENTE_PAGA") {
        totalPendente += saldo;
        if (new Date(p.data_vencimento) < hoje) totalVencido += saldo;
      }
      if (p.status === "PAGA" && p.data_pagamento && new Date(p.data_pagamento) >= inicioMes) {
        totalPagoNoMes += pago;
      }
    }
  }
  return { totalPendente, totalVencido, totalPagoNoMes, totalContas: contas.length };
}
...
export function ContasPagarTab() {
  ...
  const fetchContas = useCallback(() => {
    setLoading(true);
    request("/financeiro/contas-pagar/", { headers: authHeaders() })
      .then((res) => res.json())
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter((c) => c.fornecedor_nome?.toLowerCase().includes(q) || c.numero_documento?.toLowerCase().includes(q));
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      <KPICards cards={[
        ...
        { label: "Total Pago", value: formatCurrency(kpis.totalPagoNoMes), icon: CheckCircle, iconColor: "text-emerald-500", iconBg: "bg-emerald-50 dark:bg-emerald-950/40" },
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-b">
              <TableHead className="text-xs font-semibold text-foreground h-10">Fornecedor</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={conta.public_id} className="hover:bg-muted/30 transition-colors h-14">
                <TableCell className="font-semibold text-sm py-3">{conta.fornecedor_nome}</TableCell>
                ...
                <TableCell className="hidden md:table-cell font-mono text-sm py-3">
                  {`${(conta.parcelas ?? []).filter((p) => p.status === "PAGA").length}/${conta.total_parcelas}`}
                </TableCell>
      ...
      <DetalhesContaPagarModal conta={detalheConta} onClose={() => setDetalheConta(null)} />
      <EditContaPagarModal conta={editConta} onClose={() => setEditConta(null)} onSuccess={fetchContas} />
      <EditContaPagarModal conta={null} isNew open={showNew} onClose={() => setShowNew(false)} onSuccess={fetchContas} />
    </div>
  );
}